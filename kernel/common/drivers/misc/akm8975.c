...
#endif
#if AKM8975_DEBUG_DATA
	printk(KERN_INFO "akm8975 AKECS_SetYPR\n ");
...
#endif
#if 0
	printk(KERN_INFO "AKM8975 %s: flag =0x%X", __func__,
		rbuf[0]);
...
static ssize_t akm8975_accel_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}
...
static ssize_t akm8975_sysfs_accel_store(
	struct akm8975_data *akm, char const *buf, size_t count)
{
	int en = 0;
	...
	//printk(KERN_INFO "akm8975_sysfs_accel_store: buffer:%s   size=%d", buf, count);
...
static ssize_t akm8975_accel_store(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	return akm8975_sysfs_accel_store(
...
//	printk(KERN_INFO "akm8975_bin_accel_write++: %x", akm);
	//printk(KERN_INFO "akm8975_bin_accel_write++: buffer:%s   size=%d", buf, size);
...
//	printk( KERN_INFO "akm8975_bin_accel_write Alok accel:%d,%d,%d\n",
//		accel_data[0], accel_data[1], accel_data[2]);
//	printk(KERN_INFO "akm8975_bin_accel_write--");
...
static irqreturn_t akm8975_irq(int irq, void *handle)
{
	...
	int err;
	//printk(KERN_INFO "akm8975_irq: got interrupt\n");
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev, "akm8975_probe: start probing.");
