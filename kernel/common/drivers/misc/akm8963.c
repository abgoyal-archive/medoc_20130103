...
#define GPIO_COMPASS_RESET 22

#if AKM8963_DEBUG_DATA
#define AKM_DATA(dev, msg...)  dev_dbg((dev), ##msg)
...
struct akm8963_data {
	...
	unsigned char mode;
	int reset_gpio;////added by xqj,for reset 
	unsigned char reset_flag;
};

//add by wangpl, for bug259001
#ifdef CONFIG_REGULATOR
static struct regulator *compass_regulator = NULL;
#endif
...
//printk( KERN_INFO "TxData: len=%02x, addr=%02x data=%02x",
...
static int AKECS_ioctl(struct inode *inode, struct file *file,
	...
	unsigned char mode)
{
	int err;
	int ret;
	//printk(KERN_INFO "AKECS_SetMode %d", mode);
	ret = gpio_get_value(s_akm->reset_gpio);
	//printk(KERN_INFO "akm8963 GPIO_COMPASS_RESET=%d,reset_flag=%d\n ",ret,s_akm->reset_flag);
	if(ret==0)//avoid other place make GPIO reset pin LOW
	{
               gpio_set_value(s_akm->reset_gpio, 0);
	        udelay(120);
	        gpio_direction_output(s_akm->reset_gpio, 1);
	        mdelay(5);
	}
	switch (mode) {
...
{
       //gpio_direction_output(GPIO_OUT_33, 1);
	//udelay(10);
	int ret;
	ret = gpio_get_value(s_akm->reset_gpio);
	printk(KERN_INFO "akm8963 GPIO_COMPASS_RESET=%d,reset_flag=%d\n ",ret,s_akm->reset_flag);
       ...
       gpio_set_value(s_akm->reset_gpio, 0);
	udelay(120);
	gpio_set_value(s_akm->reset_gpio, 1);
	...
	int *rbuf)
{
...
#if 0  //old boardcom platform driver
...
#endif
#if AKM8963_DEBUG_DATA
	printk(KERN_INFO "akm8963 AKECS_SetYPR\n ");
	...
	/* Report acceleration sensor information */
	if (ready & ORI_DATA_READY) {
		input_report_abs(akm->input, ABS_HAT0X, rbuf[0]);
		input_report_abs(akm->input, ABS_HAT0Y, rbuf[1]);
		input_report_abs(akm->input, ABS_HAT1X, rbuf[2]);
		input_report_abs(akm->input, ABS_HAT1Y, rbuf[4]);
	}
#if 0
	/* Report magnetic vector information */
	...
	/* Report orientation sensor information */
	if (ready & MAG_DATA_READY) {
		input_report_abs(akm->input, ABS_RX, rbuf[9]);
		input_report_abs(akm->input, ABS_RY, rbuf[10]);
		input_report_abs(akm->input, ABS_RZ, rbuf[11]);
...
static int
AKECS_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
			unsigned long arg)
{
...
static ssize_t akm8963_accel_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}
...
static ssize_t akm8963_sysfs_accel_store(
	struct akm8963_data *akm, char const *buf, size_t count)
{
	int en = 0;
	...
	//printk(KERN_INFO "akm8963_sysfs_accel_store: buffer:%s   size=%d", buf, count);
...
static ssize_t akm8963_accel_store(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	return akm8963_sysfs_accel_store(
...
//add by wangpl, for bug259001
#ifdef CONFIG_REGULATOR
	if (!IS_ERR_OR_NULL(compass_regulator)){
		regulator_disable(compass_regulator);
//		printk("\n\n-------- by wangpl -------, in func <%s>, after regulator_disable\n\n", __func__);
	}
#endif
...
#ifdef CONFIG_REGULATOR
	if (!IS_ERR_OR_NULL(compass_regulator)){
		regulator_enable(compass_regulator);
//		printk("\n\n-------- by wangpl -------, in func <%s>, after regulator_enable\n\n", __func__);
	}
#endif
	...
	wake_up(&s_akm->open_wq);
      if(s_akm->reset_flag==0 && (ready & ORI_DATA_READY)&& (ready & MAG_DATA_READY))
	    akm8963_reset(s_akm);
...
//add by wangpl, for bug259001
#ifdef CONFIG_REGULATOR
	compass_regulator = regulator_get(NULL, "vddo_cp");
	if (!IS_ERR_OR_NULL(compass_regulator)){
		regulator_enable(compass_regulator);
//		printk("\n\n-------- by wangpl -------, in func <%s>\n\n", __func__);
	}
	...
	/*xqj:this function is mainly for  GPIO reset init , and pull reset gpio high
	it should be run before using I2C, so it shuld be move forward ,this is not same as poppy*/
	pdata->init();
	if (pdata) {
		/* Platform data is available. copy its value to local. */
		s_akm->layout = pdata->layout;
		s_akm->reset_gpio = pdata->reset;
	} else {
		...
		s_akm->layout = 0;
		s_akm->reset_gpio=GPIO_COMPASS_RESET;
	}
	...
	bak_mode=AK8963_MODE_POWERDOWN;
       ...
       gpio_set_value(s_akm->reset_gpio, 0);
	udelay(120);
	err = gpio_direction_output(s_akm->reset_gpio, 1);
	if (err < 0) {
		dev_err(&client->dev,
		       "akm8963_probe: gpio_direction_output reset gpio failed\n");
		goto exit6;
	}
...
#ifdef CONFIG_REGULATOR
	if (!IS_ERR_OR_NULL(compass_regulator)) {
		regulator_put(compass_regulator);
		compass_regulator = NULL;
	}
#endif
