...
#define uart_has_tty(po)     	((po)->state->port.tty)
#define uart_has_no_tty(po)	(!uart_has_tty(po))
...
static void transmit_chars(struct uart_8250_port *up)
{
	...
	if (!(up->mcr& UART_MCR_AFE) && uart_has_tty(&up->port) && uart_tx_stopped(&up->port)) {
...
#if defined(MOD_8250_FOR_BRCM)
		if (!serial_ucr_pdstate(up)) serial_ucr_pden(up, 0);
#endif
...
static void serial8250_backup_timeout(unsigned long data)
{
	...
	up->lsr_saved_flags |= lsr & LSR_SAVE_FLAGS;
	spin_unlock_irqrestore(&up->port.lock, flags);
	if (!uart_circ_empty(&up->port.state->xmit) && uart_has_tty(&up->port) && (uart_tx_stopped(&up->port)))
		check_modem_status(up);
		       ...
		       struct ktermios *old)
{
	...
	/*
	 * Loose defense to avoid uart_resume_port() bug, Harry@Jan.05.2012
	 */
	if (uart_has_no_tty(&up->port))
		return;
#endif
	      ...
	      unsigned int oldstate)
{
	...
	serial8250_set_sleep(up, state != 0);
#if defined(MOD_8250_FOR_BRCM)
	if (state != 0) {
		clk_disable(up->clk);
	} else {
...
serial8250_console_write(struct console *co, const char *s, unsigned int count)
{
	...
	int locked = 1;
#if defined(MOD_8250_FOR_BRCM)
	int pdstate = !!serial_ucr_pdstate(up); /* PWR_STATE on or not */
#endif
	touch_nmi_watchdog();
...
#if defined(MOD_8250_FOR_BRCM)
	/*
	 * In case UART in low-power state then deassert PWR_DIS.
	 * This is considering ttySx being opened, Harry@Jan.05.2012
	 */
	if (!pdstate) serial_ucr_pden(up, 0);
#endif
	...
	if (!pdstate) {
		/*
		 * Set automatic power-down enable, then wait UART enter low power state
		 * This is to avoid printk() path losing characters, Harry@Jan.05.2012
		 */
		serial_ucr_pden(up, 1);
		do {
			serial_in(up, UART_RX);
