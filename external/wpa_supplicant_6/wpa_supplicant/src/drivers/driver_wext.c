...
static int wpa_driver_wext_set_probe_req_ie(void *priv, const u8 *ies,
					size_t ies_len)
{
	struct wpa_driver_wext_data *drv = priv;
	struct iwreq iwr;
	int ret = 0, flags, buf_len;
	char *buf;

	buf_len = WPS_PROBE_REQ_IE_CMD_LENGTH + ies_len;
	buf = (char *)malloc(buf_len);
	if (!buf) {
		wpa_printf(MSG_ERROR, "no Memory");
		return -1;
	}

	os_memset(buf, 0, buf_len);
	if (ies_len != 0) {
		os_strncpy(buf, WPS_ADD_PROBE_REQ_IE_CMD, WPS_PROBE_REQ_IE_CMD_LENGTH);
	} else {
		os_strncpy(buf, WPS_DEL_PROBE_REQ_IE_CMD, WPS_PROBE_REQ_IE_CMD_LENGTH);
	}
	os_memcpy(&buf[WPS_PROBE_REQ_IE_CMD_LENGTH], ies, ies_len);
	os_memset(&iwr, 0, sizeof(iwr));
	os_strncpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	iwr.u.data.pointer = buf;
	...
	if (ret < 0) {
		wpa_printf(MSG_ERROR, "%s failed (%d)", __func__, ret);
	}
