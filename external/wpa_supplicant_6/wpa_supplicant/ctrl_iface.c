...
#endif
	pos = wpa_supplicant_wps_ie_txt(wpa_s, pos, end, res);
#ifdef WAPI
	if (!ie && !ie2 && !ie3 && res->caps & IEEE80211_CAP_PRIVACY) {
#else
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		...
		ret = wpa_supplicant_ctrl_iface_scan_result(wpa_s, res, pos,
							    end - pos);
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
		pos += ret;
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpa_supplicant_ctrl_iface_stop_wps(wpa_s))
			reply_len = -1;
#endif /* CONFIG_WPS */
