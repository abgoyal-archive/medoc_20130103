...
#ifdef CONFIG_WPS2
static void parse_property(struct wpa_config *config, char* name, char* property)
{
	char buf[PROPERTY_VALUE_MAX];
	int line_length = 255;
	char line[line_length];
	if (property_get(property, buf, "") > 0)
	{
		snprintf(line, line_length, "%s=%s", name, buf);
		wpa_config_process_global(config, line, 0);
	}
}
static void read_wps_config(struct wpa_config *config)
{
	parse_property(config, "device_name", "ro.product.device");
	parse_property(config, "manufacturer", "ro.product.manufacturer");
	parse_property(config, "model_name", "ro.product.name");
	parse_property(config, "model_number", "ro.product.model");
	parse_property(config, "serial_number", "ro.serialno");
}
#endif /* CONFIG_WPS2 */
