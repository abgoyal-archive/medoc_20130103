...
void HTMLObjectElement::parseMappedAttribute(MappedAttribute *attr)
{
    ...
    if (attr->name() == typeAttr) {
        ...
        if (!isImageType() && m_imageLoader)
          m_imageLoader.clear();
    } else if(attr->name() == codebaseAttr) {               //Add by wei.cao for bug 238813.
       m_codeBase = deprecatedParseURL(val);
    } else if (attr->name() == dataAttr) {
        m_url = deprecatedParseURL(m_codeBase + val);  //Modified by wei.cao for bug 238813.
        if (renderer())
          m_needWidgetUpdate = true;
